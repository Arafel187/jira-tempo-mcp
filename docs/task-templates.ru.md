...
Устаревшее поле `project_key` в файле шаблона **игнорируется** — поле
удалено из схемы, потому что целевой проект всегда передаётся при вызове
(аргумент `project_key` у `create_issue_from_template`). Существующие
файлы шаблонов с этим полем продолжают загружаться.
